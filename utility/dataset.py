...
class CorrelatedNoise(object):
    def __init__(self, beta = 23.08, eta = 0.157, bands = 31, **kwargs):
    ...
    def __call__(self, img):
        noise = self.sigmas * torch.randn(*img.shape)
        return img + noise.numpy()
...
class HSI2Tensor(object):
    ...
    def __call__(self, hsi):
        ...
        # img = torch.from_numpy(hsi)
        # print(noise_level)
        return img.float(),noise_level.float()


class GTHSI2Tensor(object):
    """
    Transform a numpy array with shape (C, H, W)
    into torch 4D Tensor (1, C, H, W) or (C, H, W)
    """
...
class ImageTransformDataset(Dataset):
    ...
    def __getitem__(self, idx):        
        ...
        if self.transform is not None:
            img, noise_level = self.transform(img)
        if self.target_transform is not None:
            target = self.target_transform(target)
        return img, target,noise_level
