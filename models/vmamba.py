...
class SS2Dv0:
    ...
    def forwardv0(self, x: torch.Tensor, seq=False, force_fp32=True, **kwargs):
        # self.in_proj = self.in_proj.to(x)
        x = self.in_proj(x)
        x, z = x.chunk(2, dim=-1) # (b, h, w, d)
        z = self.act(z)
        x = x.permute(0, 3, 1, 2).contiguous()
        # self.conv2d = self.conv2d.to(x)
        ...
        # x_proj_weight = self.x_proj_weight.to(xs)
        x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs, self.x_proj_weight)
        if hasattr(self, "x_proj_bias"):
            x_dbl = x_dbl + self.x_proj_bias.view(1, K, -1, 1)
        dts, Bs, Cs = torch.split(x_dbl, [R, N, N], dim=2)
        # dt_projs_weight = self.dt_projs_weight.to(dts)
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        if True:
            ...
            self.x_proj = self.x_proj.to(xs)
            x_dbl = self.x_proj(xs.view(B, -1, L))
            dts, Bs, Cs = torch.split(x_dbl.view(B, K, -1, L), [R, N, N], dim=2)
            dts = dts.contiguous().view(B, -1, L)
            self.dt_projs = self.dt_projs.to(dts)
...
def do_throughput(name="vmamba_tiny_s1l8", data="/media/memfs/ImageNet_ILSVRC2012/val"):
    ...
    model = VSSM_CSC(
        depths=[2, 2, 9, 2], dims=[192, 192, 192, 192], drop_path_rate=0.2, 
        patch_size=4, in_chans=3, num_classes=1000, 
        ssm_d_state=16, ssm_ratio=2.0, ssm_dt_rank="auto", ssm_act_layer="silu",
        ssm_conv=3, ssm_conv_bias=True, ssm_drop_rate=0.0, 
        ssm_init="v0", forward_type="v0", 
    ...
    input = torch.zeros([147,192,16,16]).cuda()
    out = model(input)
    print(out.shape)
    # throughput(data_loader_val, model)

class VSSM_CSC(nn.Module):
    def __init__(
        self, 
        patch_size=4, 
        in_chans=3, 
