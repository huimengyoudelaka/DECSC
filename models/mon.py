...
def init_fft_conv(weight, hw):
    ...
    return fft_to_complex_matrix(my_rfft2(kernel))
...
def fft_conv(x, w_fft, transpose=False):
    ...
    """
    x_fft = fft_to_complex_vector(my_rfft2(x))
    # 矩阵乘法感觉不太对？
...
class MONBorderReLU(nn.Module):
    def __init__(self, border=1, act = F.relu):
        super().__init__()
        self.border = border
        self.act = act

    def forward(self, *z):
        zn = tuple(self.act(z_) for z_ in z)
...
class MONMultiConv(nn.Module):
    ...
    def apply_inverse_conv(self, z, i):
        z0_fft = fft_to_complex_vector(my_rfft2(z))
    ...
    def apply_inverse_conv_transpose(self, g, i):
        g0_fft = fft_to_complex_vector(my_rfft2(g))
